...
├── src/
│   ├── __init__.py
│   ├── models.py           # Canonical Pydantic schemas (ContextItem, SearchPlan)
│   ├── connectors.py       # BaseConnector interface and source adapters
│   ├── storage.py          # Persistent ChromaDB client and metadata indexer
...
## 3. Canonical Schemas (`src/models.py`)
...
### 4.1 Ingestion & Normalization (`src/connectors.py`)

* `BaseConnector(ABC)`: Defines abstract method `fetch_records() -> list[ContextItem]`.
* `MockCalendarConnector(BaseConnector)`:

  * Parses `data/calendar.json` (fields: `id`, `summary`, `start_time`, `end_time`, `attendees`, `description`).
  * Normalizes to `ContextItem`:

    * `id`: `f"cal_{raw['id']}"`
    * `source`: `"calendar"`
    * `timestamp`: Parsed ISO datetime from `start_time`
    * `title`: `raw['summary']`
    * `content`: `f"Meeting: {raw['summary']}\nAttendees: {raw['attendees']}\nNotes: {raw['description']}"`
    * `metadata`: `{"source": "calendar", "timestamp_epoch": int(ts.timestamp()), "category": "meeting"}`
* `MockGmailConnector(BaseConnector)`:

  * Parses `data/emails.json` (fields: `id`, `thread_id`, `sender`, `subject`, `date`, `body`, `requires_action`, `category`).
  * Normalizes to `ContextItem`:

    * `id`: `f"email_{raw['id']}"`
    * `source`: `"email"`
    * `timestamp`: Parsed ISO datetime from `date`
    * `title`: `raw['subject']`
...
### 4.2 Central Context Storage (`src/storage.py`)
...
The implementation must deterministically answer the core founder workflows:

| Test Question                                         | Router Intent & Filters                                            | Primary Source          | Output Focus                                                 |
| ----------------------------------------------------- | ------------------------------------------------------------------ | ----------------------- | ------------------------------------------------------------ |
| **"What should I focus on today?"**                   | Time: Current day bounds (`$gte`, `$lte`), Action required: `True` | Calendar + Emails       | Chronological meeting priorities + urgent email deliverables |
...
## 6. CLI & Automated Execution Interface (`src/main.py`)

* **Interactive REPL**: `python src/main.py` provides an interactive shell for open-ended queries.
* **Automated Test Mode**: `python src/main.py --test` runs ingestion, executes the 3 required evaluation questions, and prints grounded responses.
